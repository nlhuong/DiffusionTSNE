...
#define INITIALIZE_TIME clock_t STARTVAR;
#define START_TIME { STARTVAR = clock(); }
#define END_TIME(LABEL) {          			                           \
  clock_t ENDVAR = clock();                                        \
  printf("%s: %4.2f ms\n",                                         \
    ((float) 1000.0*(ENDVAR - STARTVAR) / CLOCKS_PER_SEC));        \
}
#else
...
//    clock_t start, end;
    ...
    // end = clock();
    // printf("Step 1: %4.2f ms\n",
    //   ((float) 1000.0*(end - start) / CLOCKS_PER_SEC));
    // start = clock();
    END_TIME("Step 1");
    START_TIME;
